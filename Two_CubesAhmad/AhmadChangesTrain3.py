...
from Load_Batch_Data_Pytorch3 import load_data_generator
...
def intro_vae_frame_prediction(device=torch.device("cuda:0"), batch_size=32,
    ...
    optimizer_e = torch.optim.AdamW(model.encoder.parameters(), lr=0.00003)
    optimizer_d = torch.optim.AdamW(model.decoder.parameters(), lr=0.00003)
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for idx, ((img_in, _), (img_t, _), act) in enumerate(zip(img_input, img_target, vector_actions), 1):
            ...
            # ========= show info per bach ==================
                ...
                #torch.save(model.state_dict(), f'./{model_dir}/Ah3intro_vae_NAN_{epoch}_epochs')
    ...
    torch.save(model.state_dict(), f'./{model_dir}/Ah3model_intro_vae_completed_after_{num_epochs}_epochs.pt')
...
if __name__ == '__main__':
    ...
    BATCH_SIZE  = 32
    LATENT_SIZE = 32
    SAVE_PERIOD = 50  # When to save a checkpoint and sample images
