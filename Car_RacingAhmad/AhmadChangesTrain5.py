...
def plot_curves(loss_kl_z_values, loss_kl_zr_values, loss_kl_zp_values,
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'./{fig_dir}/finAh5Training_Curves.png')
    plt.show()
...
def intro_vae_frame_prediction(device=torch.device("cuda:0"), batch_size=32,
    ...
    optimizer_e = torch.optim.AdamW(model.encoder.parameters(), lr=0.00008)
    optimizer_d = torch.optim.AdamW(model.decoder.parameters(), lr=0.00008)
    ...
    if load_check:
        ...
        checkpoint = torch.load("./Model_Saved/Ah6checkpoint.pt")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        if epoch % save_period == 0:
            ...
            torch.save({'epoch': epoch,
                        ...
                        }, f'./{model_dir}/Ah6checkpoint.pt')

    # Save Model
    torch.save(model.state_dict(), f'./{model_dir}/finAh6model_intro_vae_completed_after_{num_epochs}_epochs.pt')
