...
def mse_ssim_sequence(y_prediction, t_target, sequence_len):
    ...
    ax2.legend()
    ax2.grid()
    plt.savefig(f'./Images_Result_Testing/finAh5Curve_MSE_SSIM.png')
    #plt.show()
    plt.close()
...
def plot_samples(y_prediction, t_target, mode, num_img):
    ...
    fig.savefig(f'./{fig_dir}/finAh5Sample_Result_{mode}.png')
    #plt.show()
    plt.close()
...
def plot_samples_sequence(y_prediction, t_target, mode, num_img):
    ...
    fig.tight_layout()
    #fig.subplots_adjust(wspace=0.025, hspace=0.0)
    fig.savefig(f'./Images_Result_Testing/finAh5Sample_Result_{mode}.png') #HERE
    plt.show()
    plt.close()
