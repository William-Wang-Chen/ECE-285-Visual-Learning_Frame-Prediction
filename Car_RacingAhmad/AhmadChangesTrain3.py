...
from Load_Batch_Data_Ahmad3 import load_data_generator
...
class Decoder_Net(nn.Module): #Basically the generator. 
...
def plot_curves(loss_kl_z_values, loss_kl_zr_values, loss_kl_zp_values,
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(f'./{fig_dir}/finAh3Training_Curves.png')
    plt.show()
...
def intro_vae_frame_prediction(device=torch.device("cuda:0"), batch_size=32,
    ...
    optimizer_e = torch.optim.AdamW(model.encoder.parameters(), lr=0.00004)
    optimizer_d = torch.optim.AdamW(model.decoder.parameters(), lr=0.00004)
    ...
    if load_check:
        ...
        checkpoint = torch.load("./Model_Saved/Ah3checkpoint.pt")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        for idx, ((img_in, _), (img_t, _), act) in enumerate(zip(img_input, img_target, vector_actions), 1):
            ...
            # ========= show info per bach ==================
                ...
                #torch.save(model.state_dict(), f'./{model_dir}/Ah6intro_vae_NAN_{epoch}_epochs')
            ...
            if epoch % save_period == 0 and idx == 7:
                ...
                save_image(
                    ...
                    f'./{fig_dir}/finAh3sample_epoch_{epoch}.png', nrow=16)
        ...
        if epoch % save_period == 0:
            ...
            torch.save({'epoch': epoch,
                        ...
                        }, f'./{model_dir}/Ah3checkpoint.pt')

    # Save Model
    torch.save(model.state_dict(), f'./{model_dir}/finAh3model_intro_vae_completed_after_{num_epochs}_epochs.pt')
