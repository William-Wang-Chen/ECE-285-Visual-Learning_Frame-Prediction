...
from Frame_Prediction_Intro_VAE_Network import IntroVAE
...
def plot_samples(y_prediction, t_target, mode, num_img):
    ...
    fig.savefig(f'./{fig_dir}/norm6_Sample_Result_{mode}.png')
    #plt.show()
    plt.close()
...
def mse_ssim_sequence(y_prediction, t_target, sequence_len):
    ...
    ax2.legend()
    ax2.grid()
    plt.savefig(f'./Images_Result_Testing/norm6_Curve_MSE_SSIM.png')
    #plt.show()
    plt.close()
...
def evaluation_mode():
    ...
    model_saved_path = './Model_Saved/point.pt'
