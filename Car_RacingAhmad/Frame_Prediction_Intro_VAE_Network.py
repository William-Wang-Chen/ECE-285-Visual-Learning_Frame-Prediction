...
class Decoder_Net(nn.Module):
    def __init__(self, z_dim=32):
        ...
        self.group1 = nn.Sequential(
            nn.Linear(in_features=(self.z_dim + self.action_dim), out_features=1024),
...
def intro_vae_frame_prediction(device=torch.device("cuda:0"), batch_size=32,
    ...
    # Initialize two separate optimizers for the encoder and the decoder
    ...
    # Freeze all the encoder and decoder parameters
    ...
    # Unfreeze last group in both encoder and decoder
    for param in model.encoder.group3.parameters():
        param.requires_grad = True

    for param in model.decoder.group3.parameters():
        param.requires_grad = True
    ...
    if load_check:
        ...
        checkpoint = torch.load("./Model_Saved/point.pt")
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        # At epoch 30, unfreeze the second group
        if epoch == 300:
            for param in model.encoder.group2.parameters():
                param.requires_grad = True
            for param in model.decoder.group2.parameters():
                param.requires_grad = True


        # At epoch 60, unfreeze the first group (all layers are now trainable)
        elif epoch == 600:
            for param in model.encoder.group1.parameters():
                param.requires_grad = True
            for param in model.decoder.group1.parameters():
                param.requires_grad = True
        ...
        for idx, ((img_in, _), (img_t, _), act) in enumerate(zip(img_input, img_target, vector_actions), 1):
            ...
            if epoch % save_period == 0 and idx == 7:
                ...
                save_image(
                    ...
                    f'./{fig_dir}/base_sample_epoch_{epoch}.png', nrow=16)
        ...
        if epoch % save_period == 0:
            ...
            torch.save({'epoch': epoch,
                        ...
                        }, f'./{model_dir}/point.pt')

    # Save Model
    torch.save(model.state_dict(), f'./{model_dir}/base_model_intro_vae_completed_after_{num_epochs}_epochs.pt')
