...
def mse_ssim_sequence(y_prediction, t_target, sequence_len):
    ...
    ax2.legend()
    ax2.grid()
    plt.savefig(f'./Images_Result_Testing/finAh1Curve_MSE_SSIM.png')
    #plt.show()
    plt.close()
...
def plot_samples(y_prediction, t_target, mode, num_img):
    ...
    fig.savefig(f'./{fig_dir}/finAh1Sample_Result_{mode}.png')
    #plt.show()
    plt.close()
...
def plot_samples_sequence(y_prediction, t_target, mode, num_img):
    ...
    fig.tight_layout()
    #fig.subplots_adjust(wspace=0.025, hspace=0.0)
    fig.savefig(f'./Images_Result_Testing/finAh1Sample_Result_{mode}.png') #HERE
    plt.show()
    plt.close()
...
def evaluation_mode():
    ...
    model_saved_path = './Model_Saved/model_intro_vae_completed_after_1000_epochs.pt'
